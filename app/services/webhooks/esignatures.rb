...
message: 'Student Enrollment Agreement is complete.', data: @params
        ...
        document.file.attach(io: URI.open(@params.dig(:data, :contract_pdf_url)), 
filename: 'student-enrollment-agreement.pdf', content_type: 'application/pdf')
        document.save
...
message: 'Student declined to sign the Student Enrollment Agreement.', data: @params
        when 'error'
          person.notes.create note_type: 'other-event', 
message: "Error signing Student Enrollment Agreement:\n`#{@params.dig(:data, :error_code)}`", data: @params
        end
      end
    end
  end
