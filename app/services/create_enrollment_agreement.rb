...
class CreateEnrollmentAgreement
  ...
  def call
    ...
    response =
      HTTP
        .basic_auth(user: Rails.application.credentials.esignatures_api_key, pass: '')
        .post('https://esignatures.io/api/contracts', json: document_data)
        .parse
    @program_acceptance.update(
      enrollment_agreement_url: response.dig('data', 'contract', 'signers').first['sign_page_url'],
    )
  end
